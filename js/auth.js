...
window.login = async function() {
  ...
  try {
    ...
    // CRITICAL FIX: Fetch user name from Firestore and store in localStorage
    try {
      const userRef = doc(db, 'users', user.uid);
      const userSnap = await getDoc(userRef);
      if (userSnap.exists()) {
        const userData = userSnap.data();
        localStorage.setItem('userName', userData.name || user.displayName || 'Student');
        console.log('User name stored in localStorage:', userData.name);
      }
    } catch (e) {
