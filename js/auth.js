...
window.register = async function() {
  ...
  try {
    ...
    // CRITICAL FIX: Reload user to get updated profile data with displayName
    await user.reload();
    
    // Verify displayName was set
    const updatedUser = auth.currentUser;
    console.log('After updateProfile, displayName:', updatedUser.displayName);
    
    // CRITICAL FIX: Create user document in Firestore with error handling
    try {
      ...
      showError('regErrorMsg', 'Account created but failed to save user data. Please contact support.');
...
window.login = async function() {
  ...
  try {
    ...
    // CRITICAL FIX: Fetch user name from Firestore BEFORE redirecting
    try {
      ...
      if (userSnap.exists()) {
        ...
        console.log('Login: User name stored in localStorage:', userName);
      } else {
        // No user document exists, use email as fallback
        localStorage.setItem('userName', user.displayName || email.split('@')[0]);
        localStorage.setItem('userEmail', user.email);
        console.log('Login: No user doc found, using fallback');
      }
    } catch (e) {
    ...
    // NOW redirect after localStorage is set
