...
// CRITICAL FIX: Make auth state handler async to properly fetch user data
auth.onAuthStateChanged(async user => {
  ...
  // CRITICAL FIX: Always fetch user data from Firestore first
  ...
  try {
    ...
    if (userSnap.exists()) {
      ...
      console.log('Fetched name from Firestore:', displayName);
  ...
  // If Firestore didn't have name, try auth displayName
  if (!displayName && user.displayName) {
    displayName = user.displayName;
    console.log('Using displayName from auth:', displayName);
  }
  
  // If still no name, try localStorage (from previous session)
  if (!displayName) {
  ...
  if (!displayName && user.email) {
    ...
    console.log('Using email as name:', displayName);
  }
  ...
  // Now initialize UI with the correct name
  initializeQuizUser(displayName);
});

function initializeQuizUser(displayName) {
  // Update UI elements
  const userNameEl = document.getElementById('userName');
  const welcomeNameEl = document.getElementById('welcomeName');
  
  if (userNameEl) userNameEl.textContent = displayName;
  if (welcomeNameEl) welcomeNameEl.textContent = `Welcome, ${displayName}!`;
...
async function submitQuiz() {
  ...
  // CRITICAL FIX: Fetch user name fresh from Firestore for submission
  let userName = null;
  ...
  try {
    ...
    if (userSnap.exists()) {
      ...
      console.log('Submit: Fetched name from Firestore:', userName);
    }
  } catch (err) {
    console.error('Submit: Error fetching user name:', err);
  }
  
  // Fallbacks if Firestore fetch failed
  if (!userName) userName = currentUser.displayName;
  if (!userName) userName = localStorage.getItem('userName');
  if (!userName) userName = currentUser.email?.split('@')[0];
  if (!userName) userName = 'Anonymous';
  
  console.log('Submit: Final userName:', userName);
