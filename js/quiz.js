...
auth.onAuthStateChanged(user => {
  ...
  // DEFENSIVE: If displayName is missing, try to reload or use fallback
  if (!user.displayName) {
    ...
    user.reload().then(() => {
      ...
      initializeQuizUser(refreshedUser);
    }).catch(err => {
      console.error('Failed to reload user:', err);
      initializeQuizUser(user); // Proceed anyway
    });
  } else {
    initializeQuizUser(user);
  }
});
...
async function submitQuiz() {
  ...
  // DEFENSIVE: Ensure we have a user name
  ...
  if (!userName) {
    ...
    try {
      ...
      if (userSnap.exists()) {
        ...
        console.log('Fetched name from Firestore:', userName);
      }
    } catch (e) {
  ...
  try {
    ...
    // CRITICAL FIX: Use setDoc with merge instead of updateDoc
    ...
    try {
      ...
      // Don't fail the whole submission if user stats update fails
      // The attempt is already saved
    }
